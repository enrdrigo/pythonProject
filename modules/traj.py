import numpy as np
import pickle as pk
from modules import computepos
...
def computekftnumba(root, Np, L, posox, nk, ntry, natpermol):
    ...
    with h5py.File(root + 'dump.h5', 'r') as dump:
        ...
        for i in range(lenght + 1, len(snap)):
            ...
            poschO, pos = computepos.computeposmol(Np, datisnap.transpose(), posox, natpermol)

            ch_at, pos_at = computepos.computeat(Np, datisnap.transpose(), poschO, pos)

            en_at, posatomic, em, endip = computepos.computeaten(Np, datisnap.transpose(), pos)
